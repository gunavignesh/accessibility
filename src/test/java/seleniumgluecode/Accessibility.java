...
public class Accessibility {
	...
	public void excelwrite() throws Throwable {
		...
		for (int i = 0; i < eCount; i++) {
			...
			String vKey = driver.findElement(By.xpath("//*[@id=\"HTMLCS-msg-detail-" + i + "\"]/div[1]/div[2]/a[2]")).getText();
			int vCol = Integer.parseInt(data.getData("sColumn"));
			String[] vSeva = data.compareExcel(vKey, vCol, workingDir + data.getData("filePath"), data.getData("fileRName"), data.getData("vioSName"));
			String vSev = vSeva[4];
			String vComvio = vSeva[3];
			String vVio = vSeva[2];
			String vPath = driver.findElement(By.xpath("//*[@id=\"HTMLCS-msg-detail-" + i+ "\"]/div[@class='HTMLCS-issue-source']/div[@class='HTMLCS-issue-source-inner']")).getText();
			String vNotes = driver.findElement(By.xpath("//*[@id=\"HTMLCS-msg-detail-" + i + "\"]/div[1]/div[1]")).getText();
			String[] valueToWrite = { url, wTitle, vKey, vPath, vNotes, vSev, vVio, vComvio };
			data.writeExcel(workingDir + data.getData("filePath"), data.getData("fileWName"), data.getData("vioSName"),valueToWrite);
